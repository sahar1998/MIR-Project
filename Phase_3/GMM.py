...
gmm = GaussianMixture(n_components=60,verbose=1, covariance_type='diag')
gmm.fit(response1)
labels = gmm.predict(response1)

with open('tfidf-GMM.csv', 'w', newline='') as file:
...
gmm = GaussianMixture(n_components=60,verbose=1, covariance_type='diag')
gmm.fit(response2)
labels = gmm.predict(response2)

with open('W2V-GMM.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["id", "cluster"])
    for i in range(1,len(documents_list)):
        writer.writerow([documents_id_list[i], labels[i]])
